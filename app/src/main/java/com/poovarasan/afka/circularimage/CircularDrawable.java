...
import com.poovarasan.afka.circularimage.notification.NotificationDrawer;
...
public class CircularDrawable extends Drawable {
    ...
    public void setBitmapOrTextOrIcon(List<Object> sources) throws IllegalArgumentException {
        sourceObjects.clear();
        for (int i = 0; i < sources.size() && i < 4; i++) {
            if (sources.get(i) instanceof Bitmap) {
                Bitmap                                      bitmap = (Bitmap) sources.get(i);
                BitmapDrawer drawer = new BitmapDrawer();
                drawer.bitmap = bitmap;
                drawer.bitmapShader = new BitmapShader(bitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
                sourceObjects.add(drawer);
            }
            else if (sources.get(i) instanceof BitmapDrawer) {
                BitmapDrawer drawer = (BitmapDrawer) sources.get(i);
                drawer.bitmapShader = new BitmapShader(drawer.bitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
                sourceObjects.add(drawer);
            }
            else if (sources.get(i) instanceof TextDrawer) {
                sourceObjects.add(sources.get(i));
            } else if (sources.get(i) instanceof IconDrawer) {
                sourceObjects.add(sources.get(i));
            } else {
                throw new IllegalArgumentException("Arguments can either be instance of Bitmap or TextDrawer or IconDrawer");
            }
